...
const dummyThumbnail = 'http://coinrevolution.com/wp-content/uploads/2020/06/cryptonews.jpg';
      ...
      {!simplified && (
        <Col span={24} style={{textAlign: 'center', paddingTop: 20}}>
          <Select showSearch className="select-news" placeholder="Select a Crypto" optionFilterProp="children" onChange={(value) => setNewsCategory(value)} filterOption={(input, option) => option.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}>
            ...
            {data?.data?.coins.map((coin, i) => <Select.Option value={coin?.name} key={i}>{coin.name}</Select.Option>)}
          </Select>  
        </Col>
      )}
          ...
          <Card hoverable className="news-card">
            <a href={news.url} target="_blank" rel="noreferrer">
              <div className="news-image-container">
                ...
                <img style={{ maxWidth: '100px', maxHeight: '100px' }} src={news?.image?.thumbnail?.contentUrl || dummyThumbnail} alt="news"/>
              </div>
              <p>
              ...
              <div className="provider-container">
                <div>
                  <Avatar src={news.provider[0]?.image?.thumbnail?.contentUrl || dummyThumbnail} alt="provider"/>
